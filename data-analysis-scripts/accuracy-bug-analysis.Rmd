...
We are remvoing trails where the participant did not read the AI when in the AI group 
```{r}
...
## Establishing our DV as binary for bug detection accuracy
...
candidate_interactions <- c(
  ...
  "ai * seq_num",
  "exercise * seq_num"
  #"ai * has_bug",               #does AI help more in instances when a bug is present or absent
  #"exercise * has_bug"         #does exercise help more in instances when a bug is present or absent
  #"seq_num * has_bug"           #
...
## Create possible candidate fixed effect structures
...
if (TRUE) {
  ...
  logtime_outputdir = 
    sprintf("%s/bug_find_correct_model_summaries_frozen_re_nohasbug", outputdir)
...
fin = sprintf("%s/bug_find_correct_model_summaries_frozen_re/metrics.csv", outputdir)
...
#df$seq_num_c <- scale(df$seq_num, scale = FALSE)
#fA = (bug_found_num) ~ exercise + ai + seq_num_c + has_bug + exercise:seq_num_c + ai:has_bug + (1 | stim_src_id)

m0 = glmer(fA, family=binomial, data=df, control = glmerControl(optimizer = "bobyqa"))
summary(m0)
...
```{r}
#fB= (bug_found_num) ~ exercise + ai + seq_num + has_bug +  exercise:seq_num +  (1 | stim_src_id)
m1 =  glmer(fB, family=binomial, data=df, control = glmerControl(optimizer = "bobyqa"))
summary(m1)
```
...
m1 is NOT preferred so we choose m0:  (bug_found_num) ~ exercise + ai + seq_num + has_bug + exercise:seq_num + ai:has_bug + (1 | stim_src_id)

A likelihood ratio test indicated that including both interaction terms significantly improved model fit (χ²(2) = 6.72, p = .035), supporting retention of the interaction model. While neither interaction term individually reached conventional statistical significance, both showed meaningful effect sizes and added explanatory value when considered jointly.

Although the AI × bug interaction did not reach conventional significance (p = .078), the direction and magnitude of the effect were consistent with a meaningful behavioral difference: when bugs were present, AI assistance improved participant accuracy by approximately 2.8×. This is supported by the model comparison, which showed that including the interaction terms significantly improved model fit (p = .035).

There was a marginal interaction between exercise and trial number (p = .069), participants in the exercise condition showed performance gains across trials. This suggests that pre-task exercise may have helped sustain attention or facilitated learning across the bug-identification trials. the exercise group becomes ~1.37× more likely to be correct
